...
import com.example.zju_bumper_cars.ModelLayer.map.MatrixState;
import com.example.zju_bumper_cars.ModelLayer.map.Mountion;
import com.example.zju_bumper_cars.utils.Square;
...
public class MySurfaceView extends GLSurfaceView {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                new Thread()
                {
                    ...
                    public void run()
                    {
                        while(flag)
                        {
                            if(x>0&&x< MainActivity.WIDTH/2&&y>0&&y<MainActivity.HEIGHT/2)
                            {//向前
                                cx=cx-(float)Math.sin(direction)*1.0f;
                                cz=cz-(float)Math.cos(direction)*1.0f;
                            }
                            else if(x>MainActivity.WIDTH/2&&x<MainActivity.WIDTH&&y>0&&y<MainActivity.HEIGHT/2)
                            {//向后
                                cx=cx+(float)Math.sin(direction)*1.0f;
                                cz=cz+(float)Math.cos(direction)*1.0f;
                            }
                            else if(x>0&&x<MainActivity.WIDTH/2&&y>MainActivity.HEIGHT/2&&y<MainActivity.HEIGHT)
                            {
                                direction=direction+DEGREE_SPAN;
                            }
                            else if(x>MainActivity.WIDTH/2&&x<MainActivity.WIDTH&&y>MainActivity.HEIGHT/2&&y<MainActivity.HEIGHT)
                            {
                                direction=direction-DEGREE_SPAN;
                            }
                            try
    ...
    private class SceneRenderer implements GLSurfaceView.Renderer
    {
        ...
        public void onDrawFrame(GL10 gl)
        {
            ...
            backgroud = new Square(SquareCoords, color);
            backgroud.draw();
        }
        @Override
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config)
        {
            ...
            Constant.yArray=Constant.loadLandforms(MySurfaceView.this.getResources(), R.mipmap.land2);

            mountion=new Mountion(MySurfaceView.this,Constant.yArray,
